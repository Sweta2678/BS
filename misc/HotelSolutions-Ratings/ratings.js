AUI.add(
	...
	function(A) {
		...
		var TPL_LABEL_SCORE;
		if(typeof isCustomFnbHookCall != 'undefined' && isCustomFnbHookCall) {
			TPL_LABEL_SCORE = '{desc}';
		} else {
			TPL_LABEL_SCORE = '{desc} ({totalEntries} {voteLabel})';
		}
				...
				prototype: {
					...
					_getLabel: function(desc, totalEntries, ratingScore) {
						...
						if(namespace == 'ihg_rating_') {
							...
							if(typeof isCustomFnbHookCall == 'undefined') {
								if (totalEntries == 1) {
				...
				prototype: {
					_renderRatings: function() {
						...
						if (themeDisplay.isSignedIn()) {
							...
							if(namespace == 'ihg_rating_') {
								var yourScoreLbl = Liferay.Language.get('your-rating') + 
										'<div style="float:right; padding-top:10px; padding-left:5px" class="liferay-rating-score-ihg" >(click to rate)</div>';
						...
						if(namespace == 'ihg_rating_') {					
							var cntText = label;
							cntText = cntText.substring(cntText.indexOf("("), cntText.indexOf(")")+1);
							cntText = '<div class="liferay-rating-score-ihg" style="float: right; padding-top: 10px; padding-left: 5px;">'+ cntText + "</div>";
							label = 'Average' + cntText;
						}
					...
					_itemSelect: function(event) {
						...
						var namespace = instance.get('namespace');
						if(namespace == 'ihg_rating_') {
							bAccess = hasAccess(instance.get('classPK'));
							...
							{
								instance.ratings.clearSelection();
								instance.ratings.select(-1);
					...
					_saveCallback: function(event, id, obj) {
						...
						var namespace = instance.get('namespace');							
						if(namespace == 'ihg_rating_') {							
							callEndecaUpdate(instance.get('classPK'), json.totalEntries, json.averageScore);
						}
					}
				}
			}
				...
				prototype: {
					_renderRatings: function() {
						...
						if (themeDisplay.isSignedIn()) {
							...
							var label = '';
							if(namespace == 'ihg_rating_') {
								label = instance._getLabel('', totalEntries, averageScore);
							} else {
								if(typeof isCustomFnbHookCall != 'undefined' && isCustomFnbHookCall) {
									label = instance._getLabel(instance.get('totalScore'), totalEntries, averageScore);
								} else {
									label = instance._getLabel(description, totalEntries, averageScore);
								}
							}
					...
					_saveCallback: function(event, id, obj) {
						...
						if(typeof isCustomFnbHookCall != 'undefined' && isCustomFnbHookCall) {
							instance.ratings.set('label', ''+score);
						} else {
