...
$(function () {
	...
	$('select.sorted-by-ts-tabs').selectmenu({ style:'dropdown', width:190 });
	$('select#sorted-by-teamspaces').selectmenu({ style:'dropdown', width:190 });
	$('select#sorted-by-applications').selectmenu({ style:'dropdown', width:180 });
	$('select#sort-by-post').selectmenu({ style:'dropdown', width:210 });
	$('select#sorted-by-filter').selectmenu({ style:'dropdown', width:110 });
	$('select.sorted-by-list').selectmenu({ style:'dropdown', width:300 });
	$('select.sorted-by-month').selectmenu({ style:'dropdown', width:120 });
     $('select.sorted-by-date').selectmenu({ style:'dropdown', width:60 });
	 $('select.sorted-by-year').selectmenu({ style:'dropdown', width:80 });
     $('select.sorted-by-search_cat').selectmenu({ style:'dropdown', width:280 });
...
/* tab content Teamspace: Going Google*/
$(function(){
	$("li:lt(2)").addClass('select');
});

$(function() {
	$("#ts_inside-tab div").hide();
		 $("#Home div").show(); 
		 $("#Documents div").show();
		 ...
		 $("#Members div").show(); 
		  $("#Resources div").show();
		 $("#Links div").show();
	$("#teamspace-tabs li:first").attr("id","current"); 
	$("#ts_inside-tab div:first").fadeIn();
    
    $('#teamspace-tabs a').click(function(e) {
        e.preventDefault();        
        $("#ts_inside-tab div").hide(); 
		 $("#Home div").show(); 
		 $("#Documents div").show();
		 ...
		 $("#Resources div").show();
		 $("#Links div").show();
        $("#teamspace-tabs li").attr("id",""); 
        $(this).parent().attr("id","current"); 
        $('#' + $(this).attr('title')).fadeIn();
		if($('#Documents ul li').length==0 && $('#Documents p').length == 0) {
			$('#Documents').append("<p class='txtstyle'>0 Documents</p>");
		}
...
/* tab content Teamspace: Calender*/
...
$(function() {
	...
	/* Check passed date is past date or not*/

	function validateFutureDate(selDate)
	{
		var isFutureDate=false;
		selDate.setHours(0,0,0,0);
		var today = new Date();
		today.setHours(0,0,0,0);
		if(selDate > today  || selDate.getTime()==today.getTime() )
		{
			isFutureDate=true;
		}
		return isFutureDate;
	...
	for(var i=0;i<eventList.length;i++) {
		SelectedDates[new Date(eventList[i])] = new Date(eventList[i]);
	}
    ...
    $( "#datepicker" ).datepicker(
    		
		{
			...
			onSelect : function(dateText, instance){
				...
				var cald=new Date(dateText);
				
				if(validateFutureDate(cald) ) {
				if((SelectedDates[new Date(cald)] == 'undefined') || (SelectedDates[new Date(cald)] == undefined)){
				
				var datestrlist = dateText.split('/');
				$("#event-date-block").css('display','none');
				$("#event_title").focus();
				document.getElementById('event_title').disabled = false;
				document.getElementById('event_location').disabled = false;
				document.getElementById('event_desc').disabled = false;
				
				document.getElementById("event_hours").disabled = false;
				document.getElementById("event_mins").disabled = false;
				document.getElementById("end_ampm").disabled = false;
				document.getElementById('cal_dayevent').disabled = false;
				
				document.getElementById("start_hour").disabled = false;
				document.getElementById("start_mins").disabled = false;
				document.getElementById("start_ampm").disabled = false;
				
				document.getElementById('event_title').value = '';
				document.getElementById('event_location').value = '';
				document.getElementById('event_desc').value = '';

				document.getElementById("event_hours").selectedIndex = 0;
				document.getElementById("event_mins").selectedIndex = 0;
				document.getElementById("end_ampm").selectedIndex = 0;
				document.getElementById('cal_dayevent').checked = false;
				
				document.getElementById('calenderSubmit').style.visibility='visible';
				document.getElementById('calenderDelete').style.visibility='hidden';
				
				document.getElementById('eventId').value = '-1';
				
				document.getElementById('startDateMonth').value = datestrlist[0]-1;
				document.getElementById('startDateDay').value = datestrlist[1];
				document.getElementById('startDateYear').value = datestrlist[2];
				
				document.getElementById("start_hour").selectedIndex = 0;
				document.getElementById("start_mins").selectedIndex = 0;
				document.getElementById("start_ampm").selectedIndex = 0;
				clearValidation();
				...
				//$('#myform_errorloc').attr('style','display:none');
			},
        beforeShowDay: function(date) {
        	var result=fn_DateCompare(currentDate,SelectedDates[date]);
            ...
            if (Highlight) 
			{
			 if(result==0)
	 				{
                return [true, "todayEvent", currentDate];
	 				}
					else{
                return [true, "eventdate", Highlight];
				}
            }
...
/*	$(function() {
    $("#datepicker").datepicker();*/
      $(".todayEvent").live("mouseenter", function() {
    	  var datestr=$(this).attr('title');
			var cald=new Date(datestr);
			var month=parseInt(cald.getMonth())+1;
			var year;
			if (!jQuery.browser.msie || (jQuery.browser.msie && parseInt(jQuery.browser.version)==9)) {
				year=parseInt(cald.getYear()) + 1900;
			} else {
				year=parseInt(cald.getYear());
			}
			
			var date=cald.getDate();
			var eventdate = month + '/' + date +'/' + year;
			
			 $("#AddEvent_link").css('display','block');
			
  	 jQuery.ajax({
  	        type: "POST",
  	        url: geteventurl1,
  	        data: {"calDate":eventdate, "action":"eventList"}
  	    }).done( function(data) {
  	         jQuery("#calender_popup").html(data);
  	    }).fail( function(jqXHR, textStatus) {
  	        alert("AJAX request failed with status:" + textStatus);
  	    });
         $( "#calender_popup" ).dialog({
					width:440
				});
         
			});
	
	     $(".eventdate").live("mouseenter", function(dateText,instance) {
	    	 var datestr=$(this).attr('title');
				var cald=new Date(datestr);
				var month=parseInt(cald.getMonth())+1;
				var year;
...
$(document).on('click', function(e){
    if ($(".ui-dialog").length) {
        ...
        if (!$(e.target).parents().filter('.ui-dialog').length && !$(e.target).parents().filter('.lfr-tag-selector-popup').length && !$(e.target).hasClass('aui-textboxlistentry-close') &&  !$(e.target).parents().filter('#ui-datepicker-div').length && !$(e.target).parents().filter('.ui-datepicker-header').length  && nodeName!='#document') {
        /*	alert("testing!!..");
...
/* member contact details popup starts*/

 $(document).ready(function() {

	$(".people-search ul li div.content a").hover(function() { 
			var off = $(this).offset();
			var cur_top = off.top - 220;
			var cur_left = off.left - 70;
			console.info(cur_top+'  '+cur_left);
			$(this).parent().find(".tooltip_Wrapper").css('top',cur_top+"px");
			$(this).parent().find(".tooltip_Wrapper").css('left',cur_left+"px");
			$(".tooltip_Wrapper").hide();		
			$(this).parent().find(".tooltip_Wrapper").show(); 	
			//$(".tooltip_Wrapper", $(this)).show();
			$('.tooltip_Wrapper',$($(this).parent())).hover(function() {}, function(){  
			      $(this).fadeOut().slideUp('slow');
			});
	 },function(){
			$(".tooltip_Wrapper").hide();
	});
	/* member contact details popup ends*/
	/* Disabling request access button after first click */
	if ($(".ts_request-button input[type='submit']").attr("disabled") == "disabled")
		{
			$(".ts_request-button input").addClass('disabled');
			$(".ts_request-button input").parent().parent().parent().addClass('ts_add-doc-member_disabled');
			$(".ts_request-button input").parent().addClass('icon3_disabled');
		}
	/* Disabling request access button after first click ends*/
	 $('#calender_cancel').click(function() {
	       $( "#calenderDialog" ).dialog( "destroy" );
	    });
		/*if($('#myform_errorloc').is(':visible')){
    		$('#myform_errorloc').css({'height':'auto','font-size':'13px'})
		}
		if($('#add-link-form_errorloc').is(':visible')){
    		$('#add-link-form_errorloc').css({'height':'auto','font-size':'13px'})
		}*/
		$('#resourcestab').click(function(){
			 $(".res_form-row input:text:first").focus();
		});
  });
