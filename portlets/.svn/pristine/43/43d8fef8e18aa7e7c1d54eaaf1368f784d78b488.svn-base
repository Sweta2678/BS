...
public class PublicationFinderImpl extends BasePersistenceImpl<Publication> implements PublicationFinder
{
    ...
    /*public void clearStagingData(String chainCode)
    {
    	Session session = null;
 	    ...
 	    {
 	    session = openSession();
    	SQLQuery query = session.createSQLQuery("CALL cleanup_staging("+chainCode+")");
    	System.out.println("call SP query "+query);
    //	query.executeUpdate();
 	   }
	    catch (Exception e)
	    {
	    	LOG.error("Error while executing cleanup staging : " + e);
	    }
	    finally
	    {
		    if (session != null)
