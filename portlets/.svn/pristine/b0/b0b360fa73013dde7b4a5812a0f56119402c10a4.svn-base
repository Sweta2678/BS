...
<%@page import="com.ihg.brandstandards.util.ActionUtil"%>
...
CMSSearchCriteria stdSpecSearchCriteria = (CMSSearchCriteria) session.getAttribute("searchCriteria");
String keyword = "";
	...
	for(int i=0;i<arlSpecifications.size();i++)
	{
		Standards specification = (Standards)arlSpecifications.get(i);
		 ...
		 if(specification!=null){
	        	boolean isSearchMatches = false;
	        	if(stdSpecSearchCriteria!=null && !stdSpecSearchCriteria.isEmpty()){
        		for(String str :stdSpecSearchCriteria.getSearchCriteria()){
        			keyword = keyword+" "+str;   
        			isSearchMatches = ActionUtil.isSearchedKeywordMatch(str,specification.getTitle());
        			if(isSearchMatches){
        				specification.setSearchMatches(isSearchMatches);
            		}else{
            			List<StandardTag> stndTags = StandardTagLocalServiceUtil.getStandardTagsBystdId(specification.getStdId());
            			if(stndTags.size() > 0){
            		    for(StandardTag stndTag :stndTags){
            		       isSearchMatches = ActionUtil.isSearchedKeywordMatch(str,stndTag.getTag());
            			if(isSearchMatches){
            				specification.setSearchMatches(isSearchMatches);
            		    }
            		     }
...
%>
        ...
        <c:choose>
	     <c:when test="<%=specification.isSearchMatches()%>">
	     <td class="result_table_cell1"><img src="/BSTheme-theme/images/solo_branch.png" title="sub item" style="border-left:2px solid orange" alt="sub item icon"/><%=specification.getStdId() %></td>
		</c:when>
		<c:otherwise>
	...
	for(int i=0;i<arlGuidelines.size();i++)
	{
		...
		if (description.length()>195)
		{
 		...
 		if(description!=null){
    		...
    		for(String str :stdSpecSearchCriteria.getSearchCriteria()){
    			keyword = keyword+" "+str;  
    			isSearchMatches = ActionUtil.isSearchedKeywordMatch(str,description);
    			if(isSearchMatches){
    				guideline.setSearchMatches(isSearchMatches);
        		}else{
        			List<StandardTag> stndTags = StandardTagLocalServiceUtil.getStandardTagsBystdId(guideline.getStdId());
        			if(stndTags.size() > 0){
        		    for(StandardTag stndTag :stndTags){
        		       isSearchMatches = ActionUtil.isSearchedKeywordMatch(str,stndTag.getTag());
        			if(isSearchMatches){
...
%>
		<tr class="guideline_table_row">
         ...
         <c:choose>
	     <c:when test="<%=guideline.isSearchMatches()%>">
	      <td class="result_table_cell1"><img src="/BSTheme-theme/images/solo_branch.png" style="border-left:2px solid orange" title="sub item" alt="sub item icon"/><%=guideline.getStdId() %></td>
		</c:when>
		<c:otherwise>
 ...
 jQuery(document).ready(function () {
	  var keyword = '<%=keyword%>';
		...
		for (i = 0; i < res.length; i++) { 
		 $('.result_table_cell2').highlight(res[i],{
			  color: "#000",
			  bold: false,
			  ignoreCase: true,
			  wholeWord: true
