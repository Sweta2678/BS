...
public class QualityLeadPortlet extends BrandStandardsGemPortlet
{
	...
	private static final String MESSERMENT_CRITERIA = "MESSERMENT_CRITERIA";
	private static final String IS_MAXIMIZE= "isMaximize";
	private static final String FLAG_CHANGED_COLUMN= "flagChangedColumn";
    ...
    public void doView(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException
    {
        ...
        if(publish != null) {
			...
			boolean isMaximizeFromURL = GetterUtil.getBoolean(httpReq.getParameter("isMaximize"),false);

	        getMessermentUserPreference(renderRequest,themeDisplay.getUserId(),isMaximizeFromURL);
    ...
    public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException, PortletException
    {
        	...
        	if(stdRecords != null && stdRecords.size() > 0){        	
        		...
        		try {
					...
					if(exportColumns != null && exportColumns.length() > 0){
						/****CR08 2018 Start*****/
						List<StandardsCountry> countries=StandardsCountryLocalServiceUtil.getCountryByRegion(regionId);
						if(Validator.isNotNull(countries)  && countries.size() >0){
							int index = 0;
							for(StandardsCountry c:countries){	
							index=exportColumns.length()+1;
							JSONObject obj=JSONFactoryUtil.createJSONObject();
								obj.put("title",c.getCountryNameCode());
								obj.put("width", 100);
								obj.put("index",index);
								exportColumns.put(obj);
							}
						}
						/****CR08 2018 END*****/
						Workbook workbook = getStdRecordsInWorkbook(exportColumns, stdRecords,countries);        		
        ...
        else if("updateMessurementPreference".equals(resourceID))
        {	
        	String columnName = getHttpServletRequest(resourceRequest).getParameter("columnName");
        	String flagVal = getHttpServletRequest(resourceRequest).getParameter("flagVal");
        	updateMessurementPreferenceByUser(themeDisplay.getUserId(), columnName.equalsIgnoreCase(IS_MAXIMIZE), columnName,Boolean.valueOf(flagVal));
        }
    }
    ...
    private String saveGridChanges(ResourceRequest request, Preferences preferences)
    {
        ...
        try
        {
            ...
            //CR-14 copy to brand changes
            for (int i = 0; i< array.length(); i++)
            {
                JSONObject jsonObject = array.getJSONObject(i);
                
                if(Validator.isNotNull(jsonObject.getString("copyToBrandQLReportID"))){
                	long baseQLReportObj = jsonObject.getLong("id");
                	String copyToBrandobj = jsonObject.getString("copyToBrandQLReportID");
                	
                	notifyCopytoBrandMessageQueue(baseQLReportObj,copyToBrandobj,preferences,screenName,isGEMGlobalUser,themeDisplay.getUser());
                }
                LOG.debug("jsonObject = " + jsonObject.toString());
                list.add(new QualityLeadModel(jsonObject));
            }
    ...
    {
    	String glblMust = null, stdRegion = null, stdIdLst = null, mnlType = "258", path = null, stdStatus = null, stdTitle = null, stdCatgy = null, stdOwner = null, stdCmply = null, complDue = null,duplicateInd = null;
        ...
        if (filterRules != null && !filterRules.isEmpty()) {
        	try {
                ...
                if(Validator.isNotNull(duplicateInd)) {
                	filterMessage.append(" Duplicate:");
            ...
            for (QualityLeadModel item : stdSpecGdln)
            {
            	if(std.getDuplicateInd().equals(DUPLICATE_VALUE_NO) && item.getDuplicateInd().equals(DUPLICATE_VALUE_NO)){
					...
					if (std.getId() == item.getPrntQltyLeadRrptId()) {
						if (BSCommonConstants.STD_TYPE_SPEC.equals(item
								.getStdType())) {
							specLst.add(item);
						} else if (BSCommonConstants.STD_TYPE_GDLN.equals(item
								.getStdType())) {
							gdlnLst.add(item);
						}
						recAdded = true;
    ...
    protected Workbook getStdRecordsInWorkbook(JSONArray exportColumns, List<QualityLeadModel> stdRecords,List<StandardsCountry> countries){
        ...
        for(int i=0; i < exportColumns.length() ; i++){
        	...
        	tempCountryName=StringPool.BLANK;
        	tempCountryName = adminLanguageUtil.get("bs.country.name.cd." + columnHeader.toLowerCase());
        	if(tempCountryName != null && !tempCountryName.equalsIgnoreCase("bs.country.name.cd." + columnHeader.toLowerCase())){
        		columnHeader=tempCountryName;
        	}
        ...
        for(QualityLeadModel qlModel : stdRecords){
        	 ...
        	 populateRowValues(qlModel, columnIndexMap, dataRow, stdRowStyle,countries);
        	 
        	 for (QualityLeadModel spec : qlModel.getSpecifications()){
        		 dataRow = sheet.createRow(++rowCounter);
        		 populateRowValues(spec, columnIndexMap, dataRow, null,countries);
             }
        	 
             for (QualityLeadModel gdln : qlModel.getGuidelines()){
            	 dataRow = sheet.createRow(++rowCounter);
            	 populateRowValues(gdln, columnIndexMap, dataRow, gdlnRowStyle,countries);
             }
        }        
    	return workbook;
    }
    
    private void populateRowValues(QualityLeadModel qlModel, Map<String, Integer> columnIndexMap,  Row dataRow, CellStyle style,List<StandardsCountry> countries){    	 
    	 ...
    	 if(columnIndexMap.containsKey("Duplicate")){
    		 cellValue = StringPool.BLANK;
    		 
			 if("Y".equalsIgnoreCase(qlModel.getDuplicateInd())){
				 cellValue = "Yes";
			 }else if("N".equalsIgnoreCase(qlModel.getDuplicateInd())){
				 cellValue = "No";
			 }		 
    		 createCell(dataRow, columnIndexMap.get("Duplicate"), cellValue, style);
    	 }
    	 for(StandardsCountry c:countries){
    		 if(columnIndexMap.containsKey(c.getCountryNameCode()) && qlModel.getCountryCode().contains(c.getCountryNameCode())){
    			 createCell(dataRow, columnIndexMap.get(c.getCountryNameCode())-1, "X", style);
    		 }
    	 }
    }
    ...
    */
	private void notifyCopytoBrandMessageQueue (long baseQLReportId,String copyToBrand,Preferences preferences, String screenName,boolean isGEMGlobalUser,User user) {
		Message message = new Message();
		message.put("baseQLReportId", baseQLReportId);
		message.put("userScreenName",screenName);
		message.put("copyToBrand", copyToBrand);
		message.put("isGEMGlobalUser", isGEMGlobalUser);
		message.put("preferences", preferences);
		message.put("user", user);
		
       message.put("requestType",GemConstants.JOB_TYPE_GEM_COPY_TO_BRAND);//BSCommonConstants.MESSAGE_REQ_TYPE_CMS_BRIDGE_SYNC
       
		MessageBusUtil.sendMessage(GemConstants.GEM_COPYTOBRAND_MESSAGE_DESTINATION, message);
		if (LOG.isDebugEnabled()) {
			LOG.debug("Bridge has been notified of GEM Quality Lead Report Copy to Brand : " + copyToBrand + " Same as reportId : "+baseQLReportId);
		}
	}
	public UserSearchCriteria updateMessurementPreferenceByUser(long userId,boolean isScreenModeUpdate,String columnName,Boolean changeFlagValue){
        StringBuilder sbCriteria = new StringBuilder();
        UserSearchCriteria userCriteria = null;;
        try {
            userCriteria = UserSearchCriteriaLocalServiceUtil.getSearchCriteriaBySearchName(userId,
                    MESSERMENT_CRITERIA);
             LOG.info("after getting userCriteria from db and b4 creating object to send to db value of sbcriteria is "
                ...
                if (userCriteria != null)
                {
                    ...
                    String existingCriteria = userCriteria.getSearchCriteria();
                    if(isScreenModeUpdate){
                    	sbCriteria.append(IS_MAXIMIZE);
                    	sbCriteria.append(StringPool.COLON);
                    	sbCriteria.append(changeFlagValue);
                    	sbCriteria.append(StringPool.SEMICOLON);
                    	sbCriteria.append(existingCriteria.split(StringPool.SEMICOLON)[1]);
                    }else{
                    	String columnPreference = existingCriteria.split(StringPool.SEMICOLON)[1];
                    	String columnValues = columnPreference.endsWith(StringPool.COLON)? "":columnPreference.split(StringPool.COLON)[1];
                		List<String> result = new ArrayList<String>();
                		boolean hasColumn = false;
                			for(String col : columnValues.split(StringPool.COMMA)){
                				if(!columnName.equalsIgnoreCase(col) ){
                					if(Validator.isNotNull(col)){
                						result.add(col);
                					}
	                			}else{
	                				hasColumn = true;
                		...
                		sbCriteria.append(existingCriteria.split(StringPool.SEMICOLON)[0]);
                		sbCriteria.append(StringPool.SEMICOLON);
                    	sbCriteria.append(FLAG_CHANGED_COLUMN);
                    	sbCriteria.append(StringPool.COLON);
                		boolean isFirst=true;
                		for(String column : result){
                			if(!isFirst){
                				sbCriteria.append(StringPool.COMMA);
                			}
                			isFirst= false;
                			sbCriteria.append(column);
                		}
                	...
                	sbCriteria.append(IS_MAXIMIZE);
                	sbCriteria.append(StringPool.COLON);
                	sbCriteria.append(isScreenModeUpdate);
                	sbCriteria.append(StringPool.SEMICOLON);
                	sbCriteria.append(FLAG_CHANGED_COLUMN);
                	sbCriteria.append(StringPool.COLON);
                	if(!columnName.equalsIgnoreCase(IS_MAXIMIZE)){
                		sbCriteria.append(columnName);
                	}
                }
                
                String strCriteria = new String(sbCriteria);
        	...
        	LOG.info("Search criteria not able to save due to "+e.getMessage());
        }
       return userCriteria;
    }
	void getMessermentUserPreference(RenderRequest renderRequest,long userId,boolean isMaximizedFromURL){
		 UserSearchCriteria userCriteria;
		try {
			userCriteria = UserSearchCriteriaLocalServiceUtil.getSearchCriteriaBySearchName(userId, MESSERMENT_CRITERIA);
			if(userCriteria!=null){
	        	String[] criterias = userCriteria.getSearchCriteria().split(StringPool.SEMICOLON);
	        	renderRequest.setAttribute(IS_MAXIMIZE, isMaximizedFromURL?true:criterias[0].split(StringPool.COLON)[1]);
	        	if(!criterias[1].endsWith(StringPool.COLON)){
	        		renderRequest.setAttribute(FLAG_CHANGED_COLUMN, criterias[1].split(StringPool.COLON)[1]);
	        	}else{
	        		renderRequest.setAttribute(FLAG_CHANGED_COLUMN, "");
	        	}	        	
	        }else{
	        	renderRequest.setAttribute(IS_MAXIMIZE, isMaximizedFromURL?true:false);
	        	renderRequest.setAttribute(FLAG_CHANGED_COLUMN, "");
	        }
		} catch (SystemException e) {
			renderRequest.setAttribute(IS_MAXIMIZE, isMaximizedFromURL?true:false);
