...
public class XlsSheetIterator
{
    ...
    public List<XlsModelData> processSpreadsheet() throws InvalidFormatException, IOException
    {
        ...
        if (row != null)
        {
            ...
            if (columnCount == 1 && ( model.getProperties().containsValue("STD_ID")))
            {
            	int rowCount=1;
     ...
     * @throws IOException exception
     */
    public List<XlsModelData> processEditStateSpreadsheet() throws InvalidFormatException, IOException
    {
        List<XlsModelData> xlsModels = new ArrayList<XlsModelData>();
        Workbook workbook = WorkbookFactory.create(fileName);
        FormulaEvaluator evaluator = workbook.getCreationHelper().createFormulaEvaluator();
        ...
        if (row != null)
        {
            ...
            XlsModelData headerModel = getXlsHeader(evaluator, row, columnCount, formatter);
            xlsModels.add(headerModel);
            
            if (columnCount > 0  && (headerModel.getProperties().containsValue("STD ID") || headerModel.getProperties().containsValue("STD_ID")))
            {
            	XlsModelData rowModel = null;
                for (int rowIndx = 3; rowIndx <= rows; rowIndx++)
                {
                    row = sheet.getRow(rowIndx);
                    if (row != null)
                    {
                    	rowModel = getRowDataForEditStates(evaluator, row, columnCount, formatter);
                    	rowModel.setWorksheet(sheet.getSheetName());
                    	rowModel.setRow(rowIndx);
                        xlsModels.add(rowModel);
                    }
                }
            }
            else
    ...
    private XlsModelData getRowDataForEditStates(FormulaEvaluator evaluator, Row row, int columnCount, HSSFDataFormatter formatter)
    {
