...
//        LOG.info("THIS IS StandardsListPortlet");
        ...
        try
        {
           ...
           /* changes for make it dynamic from UI element for CR- Validation 2019*/ 
            BSLanguageUtil obsmLanguageUtil = new BSLanguageUtil(null);
            ...
            final String preferBrand, final String discontFilter)
    {
        ...
        try
        {
            ...
            arlStandards = StandardsLocalServiceUtil.getStandardsAdvancedFilterResults(criteriaMap, approverFlag, standardId,"STD", preferBrand, discontFilter);
        ...
        if(arlStandards.size() > 0){
        	boolean isSearchMatches = false;
        		for(String str :searchCriteria.getSearchCriteria()){
        			for (Standards stnd : arlStandard)
                   {
        			isSearchMatches = ActionUtil.isSearchedKeywordMatch(str,stnd.getTitle());
        			if(isSearchMatches){
        				stnd.setSearchMatches(isSearchMatches);
            		}else{
            			List<StandardTag> stndTags = StandardTagLocalServiceUtil.getStandardTagsBystdId(stnd.getStdId());
            			if(stndTags.size() > 0){
