...
<%@page import="com.ihg.brandstandards.db.model.StandardTag"%>

<%@page import="com.ihg.brandstandards.db.service.StandardTagLocalServiceUtil"%>
	 ...
	 jQuery('#addTagRow').bind("click", function(){
			jQuery('#tag-container').find('#tag-box').append($('#blank_tag_box').html());
			 $(".delete-row").click(function(event) {
				 if($('#tag-container').find(".lfr-form-row").length > 1){
						$(this).closest(".lfr-form-row").remove();
					}else{
						$(this).closest(".lfr-form-row").find('input.lfr-input-text').val("");
						} 
				});
		});
	   ...
	   $("#submit").click(function(){  
		     var addTagText = [];
		      ...
		      addTagText.push($(this).val());
		      });
		     $("#tagstextIds").val(addTagText);
		     $("#new_standard_form").submit(); // Submit the form
	 });
                                ...
                                <tr id="<%=stdImg.getImageId()%>" class="table_links_list">
                                    ...
                                    <td class="asing_img_td this_link">
                                    	<%
                                    		String userFullName = "";
                                    		User userObj = null;
                                    		long imgCreatorId = 0l;
											try {
												imgCreatorId = Long.parseLong(stdImg.getCreatorId());
												userObj = UserLocalServiceUtil.getUser(Long.parseLong(stdImg.getCreatorId()));
                                    		} catch(Exception e) {
                                    			_log.error("No user exists with user id: "+ imgCreatorId);
                                    		}
											if(userObj != null) {
...
%>
                    ...
                    <label class="wrapper_label" for="select-from" title="select from label"><span class="wrapper_span"><em class="required">* </em>Countries/Region (Add at least one)<font><span class="span_spacer"></span></font></span>

                            <div class="countries_cont_left">

                                <a href="javascript: return false;" class="move_countries" id="add_all_countries" title="move countries/region">Click here to add all countries/region</a><br/>

                                <select class="multi_add sel_countries_select" name="selectfrom" id="select-from" title="add countries/region select" multiple size="5">
                                </select>
                            ...
                            <div class="countries_cont_right">
                                ...
                                <a href="javascript: return false;" class="remove_countries" id="remove_all_countries" title="remove all countries/region">Click here to remove all countries/region</a><br/>
					...
					<%
					 List<StandardTag>standardTags = new ArrayList<StandardTag>();
					 List<StandardTag>standardTagsList = StandardTagLocalServiceUtil.getStandardTagsBystdId(standView.getStandard().getStdId());
					 if(standardTagsList.size() > 0){
						 standardTags = standardTagsList;
					 }else{
						 standardTags.add(StandardTagLocalServiceUtil.getStandardTagObject());	 
					 }
					%>
	...
	<div id="tag-box">
	<c:forEach var="standardTag" items="<%=standardTags%>">		
	 <div class="lfr-form-row lfr-form-row-inline"  style="width: 250px;height: 34px;float: left;">
		<div class="row-fields" style="width: 269px;">
		 <input class="field" id="tagstextIds" name="tagstextIds" type="hidden" value="">  
			<div class="control-group control-group-inline">
			<input class="field lfr-input-text tagText" id="tagText_1" name="tagText_1" style="max-width: 200px; " type="text" value="${standardTag.tag}" maxlength="75" >
			</div>
		</div>
		...
		<div class="clear"></div>	
            ...
            <h3><% if(!isGlobal){ %>
				...
				<span id="second_step" class="step">
                    ...
                    <label class="wrapper_label" for="support_link_country" title="standard file country"><span class="wrapper_span_char_count">Country/Region<font><span class="span_spacer"></span></font></span>                
                    ...
                    <label class="wrapper_label" for="standard_url_link_title" title="standard_url link title label"><span class="wrapper_span_char_count">Brands<font><span class="span_spacer"></span></font></span>                
                     ...
                     <label class="wrapper_label" for="standard_url_link_title" title="standard_url link title label"><span class="wrapper_span_char_count">Country/Region<font><span class="span_spacer"></span></font></span>                
...
%>
		 ...
		 <%if(standView.getStandard().getStdId() > 0){ %>               
				...
				<div class="form_navigation"> 							
                    ...
                    <input type="button" id="submit" name="submitButton" class="navigation_button" value="Submit" title="Submit form" />
                    <%  if(!isStd && standView.getStandard().getStdId() < 0) { %>
