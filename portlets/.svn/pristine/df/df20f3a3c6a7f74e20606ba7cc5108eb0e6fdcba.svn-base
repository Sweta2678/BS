										...
										<div id="${language.localeCode}--imageTranslatorDiv"
											...
											<c:forEach items="${transView.parentStdImages}"
												...
												<%
													...
													try {
														imgCreatorId = Long.parseLong(parentStdImage.getDBImage().getCreatorId());
														userObj = UserLocalServiceUtil.getUserById(PortalUtil.getCompanyId(request), imgCreatorId);
													} catch(Exception e) { 
														
														_log.error("No User Exist with User Id: "+  imgCreatorId);
													}
													if(userObj != null) {
...
<%!
	private static Log _log = LogFactoryUtil.getLog("BSAdmin-portlet.docroot.html.manageTranslations.view_jsp");
%>