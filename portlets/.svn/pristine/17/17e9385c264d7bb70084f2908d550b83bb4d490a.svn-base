...
public class GEMImportSelectionPortlet extends BrandStandardsGemPortlet {
    ...
    public void doView(RenderRequest renderRequest, RenderResponse renderResponse) throws IOException, PortletException {
        ...
        if(hasError) {
        	...
        	renderRequest.setAttribute("processLog", renderRequest.getParameter("processLog"));
		 ...
		 try {
			 ...
			 if(("validateSelection").equals(resourceID)) {
				 ...
				 if(!success){
					 ...
					 jsonObject.put("isValid", "true");
					 clearCacheAndSuccessProcess(publication, regionId, department,screenName);
				 }
				 writer.write(jsonObject.toString());
			 }	 
		 } catch (Exception e) {
		 ...
		 UploadPortletRequest uploadRequest = PortalUtil.getUploadPortletRequest(actionRequest);
	        ...
	        if(Validator.isNotNull(publication)) {	
	        	...
	        	if(Validator.isNotNull(seTemplateID) || seTemplateID!=0){
	        		 try {
	        			 String processLog = selectionProcessor.processImport(brand,regionCd,department,inputFileStream,publication,seTemplateID);
	        			 /*actionResponse.setRenderParameter("processLog",processLog);
	        			 
	        			 actionRequest.setAttribute("errorMSG", processLog);*/
	...
	private void clearCacheAndSuccessProcess(Publication publication,long regionId,long department,String screenName){
