jQuery(function(){
    ...
    groupCountryDialog = $( "#groupCountry_popup" ).dialog({
		...
		width: 717,
		 ...
		 open: function() {
		        $('#closeModal').bind('click', function() {
		            $('#groupCountry_popup').dialog('close');
		        })
		    }
	});
...
function exportReport(reportType) {
	var reportURL = exportReportURL;
	if ('exportAllReport' == reportType) {
		reportURL = exportAllReportURL;
	}
...
function groupCountryPopup(publishId, regionId, selectedGrp) {
	//load block UI
	loadProgressBar();
	$.ajax({
	  	url :groupCountryDetails,
	  	data:{'publishId':publishId,'regionId':regionId, 'groupName':selectedGrp},
	  	type: "POST",
	  	dataType: "text",
	    success: function(data) {
	    	if (data != null && data != '' && typeof data != 'undefined') {
	    		$('#groupCountry_popup').html(data);
	    		groupCountryDialog.dialog( "open" );
	    	}
	    	jQuery.unblockUI();
	    }
	 });
