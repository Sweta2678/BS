...
function getChanges()
{
	...
	if(updatedRows && updatedRows.length > 0)
	{
		...
		{
			var stdId = updatedRows[i].stdId;
			var addRow = true;
			for (k in rows)
			{
				if(stdId == rows[k].stdId)
				{
					addRow = false;
					break;
				}	
...
function endEditing(){
    ...
    if ($('#dg').datagrid('validateRow', editIndex))
    {
        ...
        if(ed != null){
	        ...
	        console.log("Combogrid data value="+copyToBrndVal);
...
$(function(){
	...
	var dg = $('#dg').datagrid({
        ...
        frozenColumns:[[
            ...
            {field:'glblMust', title:'Must', width:35,
                formatter: function (value, row) { 
                   	if (row.glblRgn == 'Global' && (row.stdType == 'STD' || row.stdType == 'SPEC'))
                	{
        ...
        columns:[[
            {field:'mnlType', title:'Manual Type', width:85, hidden:true,
            	formatter:function(value, row){
            		if (row.mnlType == '258') {
	                    return 'Operate';                
            ...
            {field:'path', title:'Path', width:110, hidden:true},
            ...
            {field:'stdType', title:'Type', width:78, 
            	styler: function(value, row, index){
	                if (value == 'STD')
	                {
            ...
            {field:'title', title:'Text', width:150,
            	formatter: function(value, row, index){ 
            		return '<span title=\"' + value + '\" class=\"easyui-tooltip\">' + value + '</span>';
            	}
            },
            {field:'catgy',title:'Category',width:65},
            {field:'stdOwner', title:'Standard Owner', width:130, hidden:true},
            {field:'cmply', title:'Compliance Date', width:120, align:'center'},
            {field:'copyToBrand', title:'Copy To Brand', width:120,
            	editor:{
	                type:'combogrid',
	                options:{
	                	panelWidth:450,
	                	...
	                	columns: [[
	                	           ...
	                	           {field:'critNm',title:'Q Criticality',width:150}
	                	       ]]                	
	                }
            	}
            },
            ...
            {field:'question', title:'Trigger Question', width:250,
            		formatter:function(value,row){
            ...
            {field:'complDue', title:'Due Date', width:105, 
            	formatter:function(value,row){
        ...
        onClickCell: function(index, field, value){
            ...
            else if (field === "glblMust")
        	{
            	...
            	if (row.glblRgn == 'Global' && row && (row.stdType == 'STD' || row.stdType == 'SPEC'))
            	{	
