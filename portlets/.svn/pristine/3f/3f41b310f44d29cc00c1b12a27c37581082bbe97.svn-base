...
$(function() {
    ...
    $("div.msgBoxButtons").click(function (e){
    	var msg = "There are error indicators on progress/summary screen. Staging can not be requested until errors are resolved.";
    	if(options.content == msg){
    		$(".ui-dialog-content").dialog("close");
    		$("#Manageaccordion #ui-accordion-Manageaccordion-panel-0 td.le button").first().attr("Disabled","Disabled")
    	}
    })
...
function generateManuals () {
	 var selRegions = [];
     $.each($('.regionSelect:checked'), function(){ 
         selRegions.push($(this).val());
     });
     
	if(selRegions.length>0){
		 $.ajax({
		  		url :generateManualsURL,
		  		data:{"publishId":publishIdValue,"pubEnv":env_val,"manualRegions":selRegions.join(",")},
		  		type: "POST",
		  		success: function(data) {
		  			window.location.href = "/web/brand-standards/bridge/manage";
		  		}, error: function (e) {
...
function openManualsDialog (publishIdVal, pubEnv) {
	if ('STAGING' == pubEnv) {
		$('#createManuals').html("Are you sure you want to regenerate all manuals? If yes, Please select the region for which you need full manuals in <u>Staging</u> for this publication?");
	} else {
		$('#createManuals').html("Are you sure you want to regenerate all manuals? If yes, Please select the region for which you need full manuals in <u>Production</u> for this publication?");
	}
	env_val = pubEnv;
	publishIdValue = publishIdVal;
