...
public class GEMMessageListener implements MessageListener 
{
    ...
    private void createUniqueGroupRequest(Message message)
    {
        ...
        if (createNotification(publishId, regionId, preferences, user, GemConstants.JOB_TYPE_COUNTRY_GROUP, groupType))
        {
            //Create a job queue entry 
            submitGroupRequest(preferences.getBrand(), preferences.getRegionCode(), regionId, publishId, 
     ...
     * @return boolean
     */
    protected boolean createNotification(long publishId, long regionId, Preferences preferences, User user, String componentName,
            String groupType)
    {
