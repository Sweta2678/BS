	    ...
	    <div class="pull-right span6 gem-btn">
		    ...
		    <c:choose>
			    <c:when test="${!showMaximize}">
		        	<a href="javascript:void(0)" onclick="resizeScreen('/web/brand-standards/gem/ql-scoring',true)" class="easyui-linkbutton" data-options="iconCls:'icon-fullscreen'" title="Maximize"></a>
		        </c:when>
		        <c:otherwise>	
		        	<a href="javascript:void(0)" onclick="resizeScreen('/web/brand-standards/gem/ql-scoring',false)" class="easyui-linkbutton" data-options="iconCls:'icon-smallscreen'" title="Minimize"></a>
		        </c:otherwise>
	        </c:choose>
		</div>
    </div>
	...
	<div class="show" id="columnSelectionDiv">
		<table style="width:100%; border:1px solid #e5e5e5 !important;">
		   	...
		   	<tr style="background:#0092dc none repeat scroll 0 0">
		   		<th colspan="6" style="text-align:left;color:#fff; font-weight:bold">Display Column Options</th>
		   	</tr>
		   	<tr>
				<td width="17%"><input id="show-mnlType" type="checkbox" onclick="togleColumn('show-mnlType', 'mnlType')"> Manual Type</td>
				<td width="17%"><input id="show-path" type="checkbox" onclick="togleColumn('show-path', 'path')"> Path</td>
				<td width="17%"><input id="show-stdStatus" type="checkbox" checked onclick="togleColumn('show-stdStatus', 'stdStatus')"> Status</td>
				<td width="17%"><input id="show-stdType" type="checkbox" checked onclick="togleColumn('show-stdType', 'stdType')"> Type</td>
				<td width="20%"><input id="show-title" type="checkbox" checked onclick="togleColumn('show-title', 'title')"> Text</td>
				<td ><input id="show-catgy" type="checkbox" checked onclick="togleColumn('show-catgy', 'catgy')"> Category</td>
			</tr>
			<tr>
				<td><input id="show-stdOwner" type="checkbox" onclick="togleColumn('show-stdOwner', 'stdOwner')"> Standard Owner</td>
				<td><input id="show-cmply" type="checkbox" checked onclick="togleColumn('show-cmply', 'cmply')"> Compliance Date</td>
				<td><input id="show-copyToBrand" type="checkbox" onclick="togleColumn('show-copyToBrand', 'copyToBrand')"> Copy To Brand</td>
				<td><input id="show-catgyhead" type="checkbox" checked onclick="togleColumn('show-catgyhead', 'catgyHead')"> Category Heading</td>
				<td><input id="show-seRptCatgy" type="checkbox" checked onclick="togleColumn('show-seRptCatgy', 'seRptCatgy')"> SE Reporting Category</td>
				<td><input id="show-crit" type="checkbox" checked onclick="togleColumn('show-crit', 'crit')"> Q Criticality</td>
			</tr>
			<tr>
				<td><input id="show-seSeqNo" type="checkbox" checked onclick="togleColumn('show-seSeqNo', 'seSeqNo')"> SE Sequence No</td> 
				<td><input id="show-question" type="checkbox" checked onclick="togleColumn('show-question', 'question')"> Trigger Question</td>
				<td><input id="show-complDue" type="checkbox" checked onclick="togleColumn('show-complDue', 'complDue')"> Due Date</td>
				<td><input id="show-duplicateInd" type="checkbox" checked onclick="togleColumn('show-duplicateInd', 'duplicateInd')"> Duplicate</td>
			</tr>
			</tbody>
		</table> 
		<hr>
...
<script type="text/javascript">
	...
	var updateMessurementPreferenceURL = parent.updateMessurementPreferenceURL;
	var flagChangedColumn = parent.flagChangedColumn;
	
	var changedColumn = flagChangedColumn.split(",");
			for(var i=0;i<changedColumn.length;i++){
				if($("#show-"+changedColumn[i]).is(":checked")){
					$("#show-"+changedColumn[i]).attr('checked', false); 
				}else{
					$("#show-"+changedColumn[i]).attr('checked', true); 
				}
			}
</script>
