$(function() {
	...
	/*Added by Dilip CR19 */
	$('#language table td, #createReportForm #dialogLanguage table td').css('text-align','inherit');
	$('#language table td, #createReportForm #dialogLanguage table td').find('input[type=checkbox]:disabled').parent().css('color','gray');
...
function openGEMReportDialog(templateId, reportType) {
    ...
    if(reportType == "Policy"){
       ...
       {
    	$('#createReportForm #dialogLanguage #languageSelection').html($('#language #languageSelection').html());
        $('#dialogLanguage').show();
        $('#createReportDialog input:checkbox').removeAttr('checked');
        
        //added by Dilip CR19 : disable languages those are not selected in measurement 
        $("#language input:checkbox").each(function(){
        	  if(this.checked != true){
        	    $("#createReportForm #dialogLanguage input#"+this.name).attr("disabled","true");
        	  }
        });
        $('#language table td, #createReportForm #dialogLanguage table td').find('input[type=checkbox]:disabled').parent().css('color','gray');
       }
...
function generateGEMReport(generateGEMReportURL) {
	...
	if(reportType != "Policy"){
		...
		$("form#createReportForm #dialogLanguage input:checkbox").each(function(){
		  if (this.checked) {langLst = langLst +','+this.name;}
		});
	}
	loadProgressBar();
	jQuery.ajax({
	    ...
	    url: generateGEMReportURL,
	  	data: {"templateId":templateId, "reportType":reportType, "reportTitle":reportTitle, "reportLanguages":langLst}
	}).done( function(data) {
		jQuery.unblockUI();
		if (data != null && data != '' && typeof data != 'undefined') {
			if(reportType == 'SA') {
...
/* Added changes for CR19 by patelhiren */
function updateLanguageSelection(localeCode,element){
	 var languageParentId = $(element).closest('table').parents().parents().parents().attr('id');
	 if(languageParentId == "language"){ 
	  languageUpdateRefreshURL = languageUpdateRefreshURL+"&updateLanguage=true";
	  var elementChecked = $('#'+localeCode).is(':checked');
	  /* load block UI */
	  loadMeasurementProgressBar();
	  $.ajax({
...
function loadMeasurementProgressBar () {
