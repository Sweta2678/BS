...
public class BridgeGlobalPublishFinderImpl extends BasePersistenceImpl<Standards> implements BridgeGlobalPublishFinder {
    ...
    public List<MustPublish> getMustPublishStandardsByIdsAndBrand(String stdIdsByCommaSeprated, String brand,
            boolean mustPublish) throws SystemException {
        String query = null;
        if (mustPublish) {
    ...
    public Map<String, Long> getTotalIssuesforMustpublish(String isMustPublish, long publishId, String brand) {
        ...
        try {
            ...
            LOG.debug("getOverallProgress: publishId = " + publishId + " brand = " + brand + " query exceute time is "
                    + (new Date().getTime() - startTime) + " mls");
    ...
    public List<BridgeGlobalPublishCountry> getCountriesByGlobalPublishIdAndRegionId(long bridgeGlobalPublishId,
            long regionId) {
        Session session = null;
        List<BridgeGlobalPublishCountry> countrylist = new ArrayList<BridgeGlobalPublishCountry>();
        String query = BridgeQueryUtils
                .getBridgeGlobalPublishCountryByGlobalPublishIdAndRegionIdQuery(bridgeGlobalPublishId, regionId);
    ...
    public Map<String, String> importGlobalMustPublishStandards(long publishId, String chainCode, String screenName,
            String stdIds) throws SystemException {
        Connection conn = null;
        ResultSet cursorResultSet = null;
        String resultIds = "";
        Workbook workbook = new HSSFWorkbook();
        CallableStatement colstmt = null;
        List<Long> std_id_list = new ArrayList<Long>();
        ...
        try {
            ...
            colstmt.registerOutParameter(5, OracleTypes.CURSOR);
            colstmt.executeQuery();

            cursorResultSet = (ResultSet) colstmt.getObject(5);
            int count = 0;
            try {
                while (cursorResultSet.next()) {
                    count++;
                    std_id_list.add(cursorResultSet.getLong(1));
                    resultIds = resultIds + (resultIds.length() > 0 ? ", " : "") + cursorResultSet.getLong(1);
                    if (count > 100) {
                        workbook = SpreadsheetUtil.createWorkbookForErrorStdIDs(cursorResultSet, std_id_list);
                        break;
                    }
                }
                File xlsFile = File.createTempFile("errorValidation", ".xls");
                FileOutputStream fos = new FileOutputStream(xlsFile);
                workbook.write(fos);
                fos.flush();
                fos.close();
                results.put("result_ids", resultIds);
                results.put("fileName", xlsFile.getName());
                results.put("recordCount", String.valueOf(count));
            ...
            LOG.debug("Global Must Publish Import: publishId = " + publishId + " chainCode = " + chainCode
                    + " query exceution time is " + (new Date().getTime() - startTime) + " mls");
        } catch (SQLException e) {
