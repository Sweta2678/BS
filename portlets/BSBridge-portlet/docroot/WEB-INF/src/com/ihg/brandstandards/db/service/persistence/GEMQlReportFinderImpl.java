...
public class GEMQlReportFinderImpl extends BasePersistenceImpl<GEMQlReport>
	...
	public List<QualityLeadModel> getGEMQlReport(String query) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				model.setCountryCode((String)obj[42]);
				records.add(model);
			}
	...
	public boolean executeSelectImportProcedure(String query) {
		long start = new Date().getTime();
		LOG.debug("ExecuteSelectImportProcedure query:" + query);
		...
		try {
			...
			success = false;
			LOG.debug("Failed update SelectImportProcedure Query: " + query);
			LOG.error("Update SelectImportProcedure", e);
		} finally {
			LOG.debug("ExecuteSelectImportProcedure took: " + (new Date().getTime() - start)
					+ "ms");
			closeConnection(conn, colstmt);
		}
		return success;
