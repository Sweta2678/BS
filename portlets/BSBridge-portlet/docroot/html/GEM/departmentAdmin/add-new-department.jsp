...
if(Validator.isNotNull(departmentId)){
	...
	deptId = String.valueOf(department.getDepartmentId());
...
<portlet:resourceURL var="validateNameURL" id='isdeptNameExist'>
    <portlet:param name="cmd" value="validateDepartment"></portlet:param>
</portlet:resourceURL>
		...
		<c:choose>
			<c:when test="${not empty departmentId}">	
				<div class="control-group">
					<label class="control-label" title="Department Id"><em class="required">* </em>Department Id:</label>
					<div class="controls">
						<input type="text" id="<portlet:namespace/>deptId" name="<portlet:namespace/>deptId" readonly="readonly" maxlength=10 style="width:200px;" value="<%=departmentId%>" class="input_text required">
					</div>
				</div>			
			</c:when>			
			<c:otherwise>
				<div class="control-group">
					<label class="control-label" title="Department Id"><em class="required">* </em>Department Id:</label>
					<div class="controls">
						<input type="text" id="<portlet:namespace/>deptId" name="<portlet:namespace/>deptId" maxlength=10 style="width:200px;" value="<%=departmentId%>" class="input_text required">
					</div>
				</div>
			</c:otherwise>
		</c:choose>
...
function submitDepartment() {
	 ...
	 var deptId = jQuery.trim($('#<portlet:namespace/>deptId').val());
	 
	 if(typeof deptId == 'undefined' || deptId == null || deptId == '' || deptId == 'null') {
		 jQuery.msgBox("Please enter Department Id.");
	 } else if(typeof deptName == 'undefined' || deptName == null || deptName == '' || deptName == 'null') {
		    ...
		    success: function(data) {
		    	if (data != null && data != '' && typeof data != 'undefined') {
		    		var parsedData = JSON.parse(data);
		    		 ...
		    		 var isIdExist = parsedData.isdeptIdExist;
			            if(isNameExist=='true' && isIdExist=='true'){
			            	jQuery.msgBox("Department Name and Id already exist. Please enter different Department Name and Id.");
			            }else if(isIdExist=='true'){
			            	jQuery.msgBox("Department Id already exist. Please enter different Department Id.");
