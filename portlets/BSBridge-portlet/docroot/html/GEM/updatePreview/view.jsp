...
<%@include file="/html/GEM/updatePreview/init.jsp" %>
...
<portlet:resourceURL var="setLevelId" id="setLevelId" />
...
<script type="text/javascript">
	...
	var attchDialog = '';
	var updateLevelId = '${setLevelId}';
	var setLevelIdDialog = '';
...
<script src="/BSBridge-portlet/js/updatepreview.js"></script>
...
<c:if test="${empty countryGroups}">
	No Unique Groups Exist. Please Generate Unique Groups.
</c:if>
<c:choose>
	...
	<div class="well well-small" style="margin-bottom: 2px;"> 
		<div class="row">
			<div class="span6">
				<div class="pull-left">
					<c:choose>
						<c:when test='${groupType eq "SA"}'>
							<span><strong>SA Unique Groups:</strong></span>
						</c:when>
						<c:when test='${groupType eq "SE"}'>
							<span><strong>SE Unique Groups:</strong></span>
						</c:when>
						<c:otherwise>
							<span><strong>Waiver Unique Groups:</strong></span>
						</c:otherwise>
					</c:choose>
					<select name="group" id="group" onchange = "javascript:loadAnotherGroup()">
		   			...
		   			<span title="${countryCode}">
			   			<c:choose>
			   				<c:when test="${not empty countryCode && fn:length(countryCode) gt 30}">
		  	...
		  	<div class="span6">
				<div class="pull-right dropdown">
					<a id="dLabel" href="#" role="button" class="btn btn-primary"  data-target="#"
					 onclick="javascript:levelIdPopup(${publishId},${regionId});">
						<i class="icon-white icon-print"></i> Countries/Region
					</a>&nbsp;
					<c:if test="${adminUser eq true}">
						<a id="dLabel" href="#" role="button" class="btn btn-primary"  data-target="#"
						 onclick="javascript:generateGroups('${groupType}');">
		...
		<liferay-ui:search-container id="searchContainer" emptyResultsMessage="No results available" searchContainer="<%=searchContainerObj%>">
			...
			<tr> 
				<c:choose>
		  		...
		  		<td colspan="2">
					...
					<c:if test='${"UPDATE" eq std.status}'>
			...
			<tr>
				...
				<td>
					...
					<c:if test='${"UPDATE" eq specStd.status}'>
			...
			<c:if test="${fn:length(std.guidelines) gt 0}">
				...
				<tr>
					...
					<td>
						...
						<c:if test='${"UPDATE" eq gdlnStd.status}'>
...
<div id="updateLevelId_popup" style="display: none;" title="Countries/Region"></div>
</c:when>
<c:otherwise>
	Not logged in !!
</c:otherwise>
