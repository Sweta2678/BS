...
<portlet:resourceURL id='supportDocsranslationProgressRefreshURL' var="supportDocsranslationProgressRefreshURL">
    <portlet:param name="requestType" value="SupportDocsTranslationProgress"></portlet:param>
...
<div align="center" class="span6">
    <table class="table table-bordered">
        <thead>
            <tr>
                <th colspan="3">
                    Supporting Documents Progress&nbsp;<i id="supportDocsTranslationProgressRefresh${tabIndx}" class="icon-refresh pull-right"></i>
                </th>
            </tr>
        </thead>
        <tbody>
            <c:forEach var="langOpt" items="${activeSecondLanguages}">
                <%
                    ...
                    Map<String, Long> supportDocsTranslationCounts = new HashMap<String, Long>();
                    String checked = "";
                    ...
                    if(request.getAttribute("supportDocsTranslationCounts") != null){
                    	supportDocsTranslationCounts = (Map<String, Long>)request.getAttribute("supportDocsTranslationCounts");
                    }
                    ...
                    if(selectedBrdgRgnLangs != null && selectedBrdgRgnLangs.get(langOpt.getKey()) != null) {
                        ...
                        selected = true;
                        if(supportDocsTranslationCounts != null && supportDocsTranslationCounts.get(langOpt.getKey()) != null) {
                            count = supportDocsTranslationCounts.get(langOpt.getKey());
                        }
                    }
                %>
                <c:if test="<%=selected%>">
	                    ...
	                    <c:choose>
							<c:when test="<%=(count > 0)%>">
								<td class="cen" width="20%">
									<a class="rightClickDisabled" href="javascript:showViewDetails('${langOpt.key}', 'AttachmentTranslationView')">
...
<script type="text/javascript">
    $("#supportDocsTranslationProgressRefresh${tabIndx}").click(function() {
        var supportDocsranslationProgressRefreshURL = '${supportDocsranslationProgressRefreshURL}';
        /* load block UI */
        loadProgressBar();
        $.ajax({
            url :supportDocsranslationProgressRefreshURL,
            type: "POST",
            dataType: "text"
        }).done(function(data) {
            if (data != null && data != '' && typeof data != 'undefined') {
                $('#supportDocsProgress${tabIndx}').html(data);
            }
            /* Un load block UI */
            jQuery.unblockUI();
        }).fail( function(jqXHR) {
