...
//    -- Trigger Q's
	...
	public static String getGemQLFormRecordsQuery(long bridgePublishId, long gemPublishId, long regionId, String regionCd, String brandCd, 
			String stdIdLst, String stdStatus, String stdOwner, long seCategory, String stdRegion, String manualType)
	{
		StringBuilder query = new StringBuilder();
        ...
        query.append("PUB_STD AS (SELECT r.std_id, 1 in_qlty_lead_rpt, r.qlty_lead_rpt_id, r.publish_id, r.gem_mgt_bucket_id, r.gem_severity_id, ");
        query.append("  r.gem_msrmnt_id, r.gem_trigger_id, r.gem_catgy_id, r.disp_order_nbr, r.creat_usr_id, r.creat_ts, r.lst_updt_usr_id, r.lst_updt_ts, ");
        query.append("  r.cmply_due_dy_qty, p.publish_id, p.chain_cd, p.publish_dt, r.rgn_id, r.gbl_must_measure_std_ind ");
        query.append("  FROM qlty_lead_rpt r, publish p ");
        query.append("  WHERE p.publish_id = ").append(gemPublishId).append(" ");
        query.append("  AND r.publish_id = p.publish_id "); 
        
        if(seCategory > 0)
        {
            query.append("AND r.gem_catgy_id = ").append(seCategory).append(" ");
        }

        if ("MEASURED".equals(stdStatus))
        {
            query.append("AND r.gem_mgt_bucket_id > 0 ");
            query.append("AND r.gem_severity_id > 0 ");
            query.append("AND r.gem_catgy_id > 0 ");
        }
        else if ("NOT_MEASURED".equals(stdStatus))
        {
            query.append("AND (r.gem_mgt_bucket_id = 0 ");
            query.append("OR r.gem_severity_id = 0 ");
            query.append("OR r.gem_catgy_id = 0) ");
        }
        else if ("GLBL_MUST_MEASURE".equals(stdStatus))
        {
            query.append("AND r.gbl_must_measure_std_ind = 'Y' ");
        }

        query.append("  AND r.rgn_id = ").append(regionId).append(" ), ");
        query.append("STD_NO_GDLN AS (SELECT s.std_id, ps.chain_cd, ps.publish_dt ");
        ...
        if (GemConstants.MANUAL_TYPE_OPERATE.equals(manualType))
        {
            query.append("AND s.std_id IN (SELECT fs.std_id FROM flag_std fs WHERE fs.flag_catgy_id = ");
            query.append(GemConstants.MANUAL_TYPE_ID_OPERATE).append(") ");
        }
        else if (GemConstants.MANUAL_TYPE_BUILD.equals(manualType))
        {
            query.append("AND s.std_id IN (SELECT fs.std_id FROM flag_std fs WHERE fs.flag_catgy_id = ");
            query.append(GemConstants.MANUAL_TYPE_ID_BUILD).append(") ");
        }
        ...
        if (stdOwner != null && !stdOwner.isEmpty())
        {
            query.append("AND s.bus_owner_nm = '").append(stdOwner).append("' ");
        }
        
        if (stdRegion != null && !stdRegion.isEmpty())
        {
            query.append("AND s.rgn_cd = '").append(stdRegion).append("' ");
        }
        query.append(" ORDER BY catgy_disp_order, gem_catgy_id, seq_disp_order, STD_ID DESC");
	...
	public static String getGemQLFormRecordsQuery(long bridgePublishId, long gemPublishId, long regionId, String regionCd, String brandCd, String seMnlTypeConfig, String filterRules)
	{
        ...
        query.append("  q.cmply_due_dy_qty, p.publish_id, p.chain_cd, p.publish_dt, q.rgn_id, q.gbl_must_measure_std_ind");
        ...
        query.append(" WHERE s.STD_ID = fs.STD_ID AND fs.flag_catgy_id=sf.flag_catgy_id GROUP BY fs.STD_ID) as FLAG_IDS");
        query.append(" FROM std s, chain_std cs, stat st, CUR_TAX TAX, TAXONOMY_STD TS, PUB_STD q, CMPLY_DATE cmply ");
        if (stdIdLst != null && !stdIdLst.isEmpty()) {
            query.append(", std_id_crit sic ");
        }
	...
	public static String getGemSaveQuery(List<QualityLeadModel> records, long publishDeptId, long parentPublishId, long publishId, long regionId, String userName, boolean isReSequence, boolean isGlobalUser){
        ...
        if(isReSequence) {
	        ...
	        query.append("  TYPE qlty_lead_rec IS RECORD (QLTY_LEAD_RPT_ID NUMBER, STD_ID NUMBER, GEM_CATGY_ID NUMBER, GEM_MGT_BUCKET_ID NUMBER, DISP_ORDER_NBR NUMBER, ROW_SEQ NUMBER);");
	        ...
	        query.append("  l_cat_invalid_seq_stds_cnt_sql VARCHAR2(1000) := 'WITH QL_REC AS (SELECT q.QLTY_LEAD_RPT_ID, q.STD_ID, q.GEM_CATGY_ID, q.GEM_MGT_BUCKET_ID, q.DISP_ORDER_NBR FROM' ||");
	        ...
	        query.append("  l_cat_invalid_seq_stds_sql VARCHAR2(1000) := 'WITH QL_REC AS (SELECT q.QLTY_LEAD_RPT_ID, q.STD_ID, q.GEM_CATGY_ID, q.GEM_MGT_BUCKET_ID, q.DISP_ORDER_NBR FROM' ||");
	        query.append("    ' QLTY_LEAD_RPT q, STD s WHERE q.STD_ID=s.STD_ID AND s.STD_TYP=''STD'' AND q.PUBLISH_ID=:l_publish_id' ||"); 
	        query.append("    ' AND q.RGN_ID=:l_rgn_id AND q.GEM_CATGY_ID=:catId AND q.DISP_ORDER_NBR > 0 ORDER BY q.DISP_ORDER_NBR, q.STD_ID DESC)' ||");
	        ...
	        query.append("    ' AND s.STD_TYP = ''SPEC'' AND q.PUBLISH_ID=:l_publish_id AND q.RGN_ID=:l_rgn_id)';");
        }
        ...
        for (QualityLeadModel std : records)
        {
            ...
            if (std.getId() > 0)
            {
                if (isGlobalUser && std.isGlobal() && "Y".equals(std.getGlobalMust()) )
                {
                    ...
                    query.append("      SELECT qlty_lead_rpt_id INTO l_qlty_lead_rpt_id FROM qlty_lead_rpt WHERE std_id = ").append(std.getStdId()).append(" AND rgn_id = l_tmp_rgn_id AND publish_id = l_publish_id;");
			...
			String saManualType, String wrManualType)
    {
        ...
        query.append("  l_sql VARCHAR2(600) := 'INSERT INTO qlty_lead_rpt(qlty_lead_rpt_id, publish_id, gem_publish_dept_id, std_id, gem_mgt_bucket_id, gem_severity_id, ' || ");
        query.append("  'gem_msrmnt_id, gem_trigger_id, gem_catgy_id, disp_order_nbr, rgn_id, cmply_due_dy_qty, gbl_must_measure_std_ind, creat_usr_id, creat_ts, lst_updt_usr_id, lst_updt_ts) ' || ");
        query.append("  'SELECT :qId, :pubId, :deptPublishId, :stdId, :bktId, :svrtId, :msrmntId, :trigId, :catId, :sortOrd, :rgnId, :cmplyDt, :glblMust, :usr, SYSDATE, :usr, SYSDATE FROM dual ' || ");
        ...
        query.append("    l_gem_trigger_id, l_gem_catgy_id, l_disp_order_nbr, rec.rgn_id, l_cmply_due_dy_qty, l_gbl_must_measure_std_ind, l_user, l_user, l_actvGemPublishId, rec.std_id, rec.rgn_id;");
        query.append("	  IF SQL%ROWCOUNT > 0 THEN");
        query.append("    	l_sa_grp_regnrt_ind := 'Y';");
	...
	public static String getApplicableBrandsByStdId(long deptId, long stdId, long regionId){
    	...
    	query.append(" 				SELECT p.PUBLISH_ID FROM PUBLISH p, GEM_PUBLISH_DEPT gpd, GEM_DEPT gd ");
	...
	public static void main(String[] args){
		System.out.println("Query=\n"+getApplicableBrandsByStdId(101, 3384, 3));
		System.exit(0);
	}
}
