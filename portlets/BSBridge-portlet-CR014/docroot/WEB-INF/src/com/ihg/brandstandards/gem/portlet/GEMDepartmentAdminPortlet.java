...
public class GEMDepartmentAdminPortlet extends MVCPortlet {
	...
	private static final Log LOG = LogFactoryUtil.getLog(GEMDepartmentAdminPortlet.class);
		 ...
		 try {
			 ...
			 if(Validator.isNotNull(deptName)&&Validator.isNotNull(deptDescription)){
				 ...
				 if(Validator.isNotNull(departmentId)){
					...
					department = DepartmentLocalServiceUtil.createDepartment(GEMTemplateLocalServiceUtil.getSequenceNumber("GEM_DEPT_ID_SEQ"));
	 ...
	 public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse){
		 ...
		 try { 
		 if(cmd.equals("validateDepartmentName")){
			 try {
			 ...
			 try {
				 department = DepartmentLocalServiceUtil.getActiveDepartmentByName(deptName, ACTIVE_VALUE);
				 if(Validator.isNotNull(department)){
				 ...
				 jsonObject.put("isdeptNameExist", "false");
				 LOG.info("No department exist with this department name"+deptName);
			 }
