...
public class XlsSheetIterator
{
    ...
    public List<XlsModelData> processSpreadsheet( List<String> requiredHeader) throws InvalidFormatException, IOException
    {
        List<XlsModelData> xlsModels = new ArrayList<XlsModelData>();
        Workbook workbook = WorkbookFactory.create(fileName);
        FormulaEvaluator evaluator = workbook.getCreationHelper().createFormulaEvaluator();
        ...
        if (cell != null)
        {
            ...
            case Cell.CELL_TYPE_FORMULA:
                ...
                default:
                    ...
                    if (LOG.isDebugEnabled())
                    {
                        LOG.debug("Unknown formula cell type" + cell.getCellType());
                    }
                    break;
                }
                break;
            ...
            default:
                ...
                if (LOG.isDebugEnabled())
                {
                    LOG.debug("Unknown cell type" + cellType);
                }
                break;
            }
        }
