...
String ckeditorHeight = PropsUtil.get("ckeditor.height.config.portlets");
	...
	<liferay-util:html-top outputKey="js_editor_ckeditor_skip_editor_loading">
		...
		<script type="text/javascript">
			...
			window.onload = function ()	{
				var editorHeight = "<%= ckeditorHeight %>";
				if (editorHeight != null && editorHeight!=""){
					var portletNames = editorHeight.split(",");
					
					for (var i = 0; i < portletNames.length; i++)
					{
						
						if ("<%= portletId %>" == portletNames[i]){
							if(CKEDITOR.instances['<%= name %>']){
								CKEDITOR.instances['<%= name %>'].config.height = '70px';			
							}
						}
					}					
				}				
