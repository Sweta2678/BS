...
<aui:form method="post" name="fm">
		...
		<div class="helper-clearfix">
			<liferay-ui:icon-menu cssClass="lfr-document-library-add-menu" icon='<%= themeDisplay.getPathThemeImages() + "/common/add.png" %>' message="add">
				...
				<c:if test="<%= ((folder == null) || folder.isSupportsMultipleUpload()) && DLFolderPermission.contains(permissionChecker, scopeGroupId, folderId, ActionKeys.ADD_DOCUMENT)%>">
					<liferay-portlet:renderURL var="editFileEntryURL" portletName="<%= PortletKeys.DOCUMENT_LIBRARY %>">
					...
					<liferay-ui:icon image="copy" message='multiple-documents' url="<%= editFileEntryURL %>" />
	...
	if (Validator.isNull(displayTerms.getKeywords())) {
		...
		/* Below code added get result by modifeid date order ---- Date:-03-24-2015*/
		String orderByCol = "modifiedDate";
		String orderByType = "desc";
		_log.debug("Custom code added to get documents in orderByCol=="+orderByCol + "   and orderByType=="+ orderByType);
		OrderByComparator orderByComparator = DLUtil.getRepositoryModelOrderByComparator(orderByCol, orderByType);
		List results = DLAppServiceUtil.getFileEntries(repositoryId, folderId, searchContainer.getStart(), searchContainer.getEnd(),orderByComparator );
