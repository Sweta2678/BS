...
<div class="article-form <%= ((article != null) && !article.isNew()) ? "article-form-edit" : "article-form-add" %>">
	...
	<aui:form action="<%= editArticleActionURL %>" cssClass="lfr-dynamic-form" enctype="multipart/form-data" method="post" name="fm1" onSubmit='<%= renderResponse.getNamespace() + "submitForm(event);" %>'>
		...
		<liferay-ui:error exception="<%= ArticleDisplayDateException.class %>" message="Please select at list one category" />
		...
		<div class="journal-article-wrapper" id="<portlet:namespace />journalArticleWrapper">
			<div class="journal-article-wrapper-content">
				...
				<c:choose>
					<c:when test="<%= Validator.isNull(toLanguageId) %>">
						<% 
							List<String> modifiedSectionsList= new ArrayList<String>();
							for (String categorySection : mainSections) {
								if(!("abstract".equalsIgnoreCase(categorySection))){
									modifiedSectionsList.add(categorySection);
								}
							}
							String[] modifiedSections = modifiedSectionsList.toArray(new String[modifiedSectionsList.size()]);
							String[][] modifiedCategorySections = {modifiedSections};
						%>
						<liferay-ui:form-navigator
							categoryNames="<%= _CATEGORY_NAMES %>"
							categorySections="<%= modifiedCategorySections %>"
							formName="fm1"
					...
					<c:otherwise>
						...
						for (String section : mainSections) {
							if(!("abstract".equalsIgnoreCase(section))){ 
						%>
