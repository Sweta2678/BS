				...
				<c:otherwise>
					...
					>
						...
						<c:if test="<%= showRecentPosts %>">
							...
							<a href="<%= recentPostsURL.toString() %>">
         								<span class="recentsPostIcon">Recents Post Icon</span>
        								 <span class="labelText">Recent Posts</span>
      					</a>
						...
						<c:if test="<%= (userDisplay != null) && (user.getUserId() != userDisplay.getUserId()) && !PortalUtil.isGroupAdmin(userDisplay, scopeGroupId) && MBPermission.contains(permissionChecker, scopeGroupId, ActionKeys.BAN_USER) %>">
							...
							<c:choose>
								<c:when test="<%= MBBanLocalServiceUtil.hasBan(scopeGroupId, userDisplay.getUserId()) %>">
									...
									<%-- <a href="<%=  unbanUserURL.toString() %>">
         								<span class="unbanIcon">unban Icon</span>
        								 <span class="labelText">Unban This User</span>
      					</a> --%>
									 <liferay-ui:icon
										image="../message_boards/unban"
										label="<%= true %>"
										message="unban-this-user"
										url="<%= unbanUserURL.toString() %>"
										cssClass ="unbanCssClass"
									/> 
								</c:when>
								<c:otherwise>
									...
									<%-- <a href="<%=  banUserURL.toString() %>">
         								<span class="banIcon">ban Icon</span>
        								 <span class="labelText">Ban This User</span>
      					</a> --%>
												
								
									 <liferay-ui:icon
										image="../message_boards/ban"
										label="<%= true %>"
										message="ban-this-user"
										url="<%= banUserURL.toString() %>"
										cssClass ="banCssClass"
									/> 
								</c:otherwise>
							</c:choose>
						</c:if>
		...
		<td class="lfr-top">
			<div class="thread-top float-container">
				<div class="subject">
					...
					<br />
					<div class="date"><%= dateFormatDateTime.format(message.getModifiedDate()) %>&nbsp; </div>
					...
					<c:if test="<%= parentMessage != null %>">
						...
						<span class="replyTo"><%= LanguageUtil.format(pageContext, "posted-as-a-reply-to", author) %></span>
					</c:if>
					<%
					String assetTagNames = (String)request.getAttribute("edit_message.jsp-assetTagNames");
					List<AssetTag> tags = AssetTagServiceUtil.getTags(MBMessage.class.getName(), message.getMessageId());
					%>
				...
				<c:if test="<%= editable %>">
					...
					<c:if test="<%= showAnswerFlag || hasReplyPermission %>">
						<ul class="edit-controls unstyled customActionMenu">
							...
							<c:if test="<%= hasReplyPermission && !thread.isLocked() %>">
								<li>
									<portlet:renderURL var="replyURL">
									 ...
									 <a href="<%= replyURL %>">
         								<span class="reply">Reply Icon</span>
        								 <span class="labelText">Reply</span>
      								  </a>
									<%-- <liferay-ui:icon
										image="reply"
										label="<%= true %>"
								...
								<li>
									<portlet:renderURL var="quoteURL">
									 ...
									 <a href="<%= quoteURL %>">
         								<span class="quoteReply">Quote Reply Icon</span>
        								 <span class="labelText">Reply with Quote</span>
      								  </a>
								...
								<li>
									...
									<a href="<%= taglibQuickReplyURL %>">
         								<span class="quickReply">Quick Reply Icon</span>
        								 <span class="labelText">Quick Reply</span>
      								  </a>
									<%-- <liferay-ui:icon
										image="bottom"
										label="<%= true %>"
			...
			<td class="thread-bottom">
				...
				<ul class="edit-controls unstyled">
					<li>
						...
						<a href="<%= topHREF %>">
         								<span class="topIcon">Top Icon</span>
					...
					<c:if test="<%= !thread.isLocked() && MBMessagePermission.contains(permissionChecker, message, ActionKeys.UPDATE) %>">
						<li>
							...
							<a href="<%= editURL %>">
         								<span class="editIcon">Edit Icon</span>
        								 <span class="labelText">Edit</span>
      					</a>
							<%-- <liferay-ui:icon
								image="edit"
								label="<%= true %>"
					...
					<c:if test="<%= (message.getParentMessageId() != MBMessageConstants.DEFAULT_PARENT_MESSAGE_ID) && MBCategoryPermission.contains(permissionChecker, scopeGroupId, category.getCategoryId(), ActionKeys.MOVE_THREAD) %>">
						<li>
							...
							<a href="<%= splitThreadURL %>">
         								<span class="splitIcon">Split Icon</span>
        								 <span class="labelText">Split Thread</span>
      					</a>
							<%-- <liferay-ui:icon
								image="unlink"
								label="<%= true %>"
					...
					<c:if test="<%= !thread.isLocked() && (thread.getMessageCount() > 1) && MBMessagePermission.contains(permissionChecker, message, ActionKeys.DELETE) %>">
						<li>
							...
							<a href="<%= deleteURL %>">
         								<span class="deleteIcon">Delete Icon</span>
        								 <span class="labelText">Delete</span>
      					</a>
