...
<%
User userDetails =PortalUtil.getUser(request) ;
long userId=userDetails.getUserId();  
long[] roles=userDetails.getRoleIds();
long[] roleIds = userDetails.getRoleIds();
List<Role> rolesList=RoleLocalServiceUtil.getRoles(roleIds);
int flag=0;
for(Role userRole:rolesList){
	if((userRole.getName().equalsIgnoreCase("IHG Admin"))||(userRole.getName().equalsIgnoreCase("TSS Admin"))){
		flag=1;
		break;
	}
}
%>
		<c:if test="<%= ((folder == null) || folder.isSupportsSocial()) %>">
			<c:if test="<%=((flag==1)||permissionChecker.isOmniadmin())%>">
