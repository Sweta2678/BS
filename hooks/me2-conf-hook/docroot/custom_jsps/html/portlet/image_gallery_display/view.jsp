...
<% 
  ...
  boolean isControlePanel=themeDisplay.getLayout().getGroup().isControlPanel();
  boolean isPhotosPageName=false;
  boolean isHotelImplementationSite = false;
    if(PropsUtil.get("conference.photos.pagename").equalsIgnoreCase(pageName)){
		isPhotosPageName=true;
	}
    if(PropsUtil.get("grs.implemetation.community").equalsIgnoreCase(themeDisplay.getScopeGroup().getName())){
    	isHotelImplementationSite=true;
	}
%> 


<c:if test="<%=!isControlePanel && isPhotosPageName%>">
  <link rel="stylesheet" type="text/css" href="/merlin-theme/css/custom_conf_photos_2.css" />
</c:if>
				...
				<aui:row>
					...
					<aui:col cssClass="lfr-asset-column lfr-asset-column-details" width="<%= showFolderMenu ? 75 : 100 %>">
						<div id="<portlet:namespace />imageGalleryAssetInfo">
							<c:if test="<%= folder != null &&  !isHotelImplementationSite%>">
...
<script type="text/javascript">
	$(function(){
		$('.portlet-image-gallery-display .taglib-header h3.header-title span:contains("CONFERENCE_PHOTOS")').parent().parent().hide();
	});
//	In conferences album, hiding unwanted divs
	$('.custom_conf_folder_wrapper').parent().prevAll().hide();
	// In conferences photos page, required background color
	$('.header-back-to').parents().find('.portlet-body').eq(0).css('background','#eee');
</script>